...
public class EmployeeController {
	...
	@RequestMapping(value = "/yash/{act}/{id}", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/yash2/{act}/{id}", method = RequestMethod.POST)
	public ModelAndView getValues(@PathVariable Map<String, String> p, @ModelAttribute("user") EmployeeDtails u) {
		...
		if (act.equals("search")) {
			...
			mv.addObject("yash", "Table After Search");
		}
		...
		if (act.equals("add")) {
			...
			mv.addObject("yash", "New Employee Table");
		}
		...
		if (act.equals("edit")) {
			a = "UPDATE employee SET employee_id= '" + u.getEid() + "',employee_name = '" + u.getEname()
					...
					+ "'";
			mv.addObject("yash", "Updated Employee Table");
		}
